...
#define REG_READ(reg, val) \
	...
	if (!priv->firmware) {
		ret = request_firmware(&priv->firmware, fw_path, priv->pdev);
		if (ret) {
			cw1200_dbg(CW1200_DBG_ERROR,
	...
	num_blocks = (priv->firmware->size - 1) / DOWNLOAD_BLOCK_SIZE + 1;

	/* Updating the length in Download Ctrl Area */
	val32 = priv->firmware->size; /* Explicit cast from size_t to u32 */
	...
	for (block = 0; block < num_blocks ; block++) {
		...
		tx_size = block_size = min((size_t)(priv->firmware->size - put),
			(size_t)DOWNLOAD_BLOCK_SIZE);

		memcpy(buf, &priv->firmware->data[put], block_size);
